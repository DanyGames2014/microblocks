...
public abstract class MicroblockItem extends TemplateItem implements EnhancedPlacementContextItem, CustomItemRenderer, CustomMultipartOutlineRenderer {
    ...
    public boolean renderOutline(PlayerEntity player, BlockPos pos, Vec3d vec, Direction side, float tickDelta){
        ...
        if(Minecraft.INSTANCE.getMultipartCrosshairTarget() != null && Minecraft.INSTANCE.getMultipartCrosshairTarget().component instanceof MicroblockMultipartComponent microblock && microblock.slot != null && microblock.canUse(player, net.minecraft.util.math.Vec3d.create(vec.getX(), vec.getY(), vec.getZ()), side, null)){
